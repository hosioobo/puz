...
> **v0.2.0**은 현재 개발 중인 라인입니다. `v0.1.0`은 첫 공개 소스 체크포인트로 남아 있습니다.

## What is puz?

puz는 macOS 로컬에서 동작하는 루틴 인터럽터입니다. 메뉴바에 있다가 다음 일정이 되면 집중된 전체화면 흐름을 열어, 휴식/운동을 눈에 띄고 단순하고 덜 쉽게 넘길 수 있게 만듭니다.

현재 앱은 일부러 작게 유지합니다. 기본 루틴 하나, 간단한 설정 창 하나, 전체화면 프롬프트 흐름 하나, 그리고 설정/완료 기록의 로컬 저장이 핵심입니다.
...
아직 signed/notarized binary는 배포하지 않습니다. 현재는 소스에서 빌드하는 방식입니다.
...
아직 서명/공증된 앱이 아니기 때문에, 로컬에서 빌드한 앱에 대해 macOS의 수동 승인 과정이 필요할 수 있습니다.

## Getting Started

### 1. Launch puz

`dist/puz.app`을 엽니다. puz는 Dock 앱이 아니라 메뉴바 앱으로 실행됩니다.

### 2. Configure your routine

설정 창에서 루틴 이름, 시간, 하루 일정, 미루기 제한을 조정합니다.
...
랜덤 구간 일정에서는 core schedule engine이 활성 루틴 설정과 현재 시각을 기준으로 다음 유효 트리거를 선택합니다.

### Completion records

완료 기록은 로컬에 저장됩니다. 런타임 스케줄링은 이 기록을 사용하되, “오늘 한 번 완료했다”는 이유만으로 이후 가능한 후보를 전부 숨기지 않도록 계산합니다.

### Future routine model

향후 모델은 더 유연해질 수 있습니다. 여러 루틴, 요일/시간대 규칙, 하루 랜덤 횟수, 최소 간격, 예외, quiet time, 더 정교한 미루기 정책을 염두에 둡니다.
...
스크린샷과 짧은 캡처 영상은 아직 넣지 않았습니다. 전체화면 흐름이 더 안정될 때까지 공개 페이지는 text-only로 둡니다.
...
- 전체화면 흐름이 안정된 뒤 스크린샷과 짧은 demo media 추가.
- 여러 루틴.
- 요일 및 다중 시간대 scheduling.
- 하루 랜덤 횟수와 최소 간격.
- 예외 / quiet time.
- 앱 언어 직접 선택.
- Black & White theme.
