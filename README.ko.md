...
현재 앱은 일부러 작게 유지합니다. 첫 실행 설정 화면, 간단한 설정 창 하나, 전체화면 프롬프트 흐름 하나, 그리고 설정/완료/건너뛰기 기록의 로컬 저장이 핵심입니다.
...
- **메뉴바 앱** — `<//>` 느낌의 작은 identity와 상태, setup, 설정 접근.
- **첫 실행 설정** — 추천 Hydrate, Stretch, Eye rest puz 카드를 고르거나, 직접 만든 최소 루틴으로 시작.
- **다중 루틴 스케줄링** — 활성 루틴이 온보딩의 지정 시각 또는 고급 요일/시간 구간 규칙에서 daily virtual slot을 만듭니다.
...
첫 실행 때 puz는 어떤 일정도 시작하기 전에 setup을 엽니다. 추천 카드 중 **Stretch**는 기본 선택되어 있고, **Hydrate**와 **Eye rest**는 선택 사항입니다. **직접 만들기**로 최소 custom routine도 만들 수 있습니다.

setup 화면은 오늘 남은 puz slot을 미리 보여줍니다. 확인하기 전에 닫으면 어떤 루틴도 활성화하지 않습니다. 메뉴바의 **설정 마치기**로 setup을 다시 열 수 있습니다.

### 3. 나중에 Settings에서 조정

setup 이후에는 Settings에서 루틴 이름, 시간, action type, 미루기 제한을 조정합니다. 활성 요일, 시간 구간, 하루 실행 횟수, 최소 간격, 배치 방식은 **고급 일정** 안에 두어 첫 사용 흐름을 작게 유지합니다.
...
현재 일정 모델은 여러 활성 루틴을 지원합니다. 추천 setup 루틴은 지정 daily slot을 사용합니다. 고급 설정에서는 활성 요일, 하나 이상의 same-day 시간 구간, 하루 실행 횟수, 최소 간격, 고르게 배치/안정적 랜덤 배치 방식을 정의할 수 있습니다.
...
- action type과 시간
- 미루기 제한
- **고급 일정**: 활성 요일, 시간 구간, 하루 실행 횟수, 최소 간격, 배치 방식
